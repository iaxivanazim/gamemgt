<x-app-layout>
    <div class="content-wrapper p-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div class="d-flex align-items-center gap-3">
                ...
                <form method="GET" action="{{ route('game_tables.index') }}" class="d-flex gap-2">
                    ...
                    <input type="text" name="search" class="form-control form-control-sm bg-black text-white border-secondary" placeholder="Search table name..." value="{{ request('search') }}" style="min-width:200px;">
                ...
                <div class="btn-group" role="group">
                    <a href="{{ route('game_tables.index', ['status' => 1, 'search' => request('search')]) }}" class="btn btn-sm {{ $status == 1 ? 'btn-warning' : 'btn-outline-warning' }}">
                        Active
                    </a>
                    <a href="{{ route('game_tables.index', ['status' => 0, 'search' => request('search')]) }}" class="btn btn-sm {{ $status == 0 ? 'btn-warning' : 'btn-outline-warning' }}">
                        Inactive
                    </a>
                </div>
        ...
        {{-- Session Flash --}}
        @if(session('success'))
        <div class="alert alert-success alert-dismissible fade show border-0 mb-4" style="background:#0f2e1a; color:#6fcf97;">
        ...
        <div class="card mb-4 border-0 overflow-hidden" style="background:#111; box-shadow: 0 0 25px rgba(255,215,0,0.07); border-left: 4px solid {{ $gameBadgeColor }} !important;">
            <div class="card-body p-0">
                <div class="row g-0">

                    {{-- ── LEFT: Felt Visual ── --}}
                    <div class="col-md-1 d-flex align-items-center justify-content-center" style="background:{{ $feltColor }}22; border-right:1px solid #222; min-height:130px;">
                    ...
                    <div class="col-md-6 p-3">
                        <div class="d-flex align-items-center gap-2 mb-1">
                            ...
                            <span class="badge rounded-pill px-2 py-1" style="background:{{ $gameBadgeColor }}33; color:{{ $gameBadgeColor }};
                                         border:1px solid {{ $gameBadgeColor }}; font-size:11px;">
                                {{ $table->gameType?->name ?? 'N/A' }}
                            </span>
                            @if($table->status == 1)
                            <span class="badge rounded-pill" style="background:#0f2e1a; color:#6fcf97; border:1px solid #6fcf97; font-size:10px;">
                                ● ACTIVE
                            </span>
                            @else
                            <span class="badge rounded-pill" style="background:#2e1010; color:#eb5757; border:1px solid #eb5757; font-size:10px;">
                                ● INACTIVE
                            </span>
                            @endif
                        </div>
                        ...
                        <div class="d-flex flex-wrap gap-3 mt-2">
                            ...
                            @if($table->float)
                            <span class="text-secondary small">
                                <i class="bi bi-cash-stack me-1"></i>Float: <span class="text-warning">{{ number_format($table->float, 2) }}</span>
                            </span>
                            @endif
                        </div>
                        ...
                        <div class="d-flex flex-wrap gap-3 mt-2">
                            <span class="small" style="color:#aaa;">
                                Min Bet: <span class="text-warning fw-bold">{{ number_format($preset->min_bet, 2) }}</span>
                            </span>
                            <span class="small" style="color:#aaa;">
                                Max Bet: <span class="text-warning fw-bold">{{ number_format($preset->max_bet, 2) }}</span>
                            </span>
                            @if(isset($preset->side_min_bet))
                            <span class="small" style="color:#aaa;">
                                Side: <span class="text-warning fw-bold">{{ number_format($preset->side_min_bet, 2) }} – {{ number_format($preset->side_max_bet, 2) }}</span>
                            </span>
                            @endif
                            @if(isset($preset->tie_min))
                            <span class="small" style="color:#aaa;">
                                Tie: <span class="text-warning fw-bold">{{ number_format($preset->tie_min, 2) }} – {{ number_format($preset->tie_max, 2) }}</span>
                            </span>
                            @endif
